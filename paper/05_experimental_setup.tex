...
\begin{tabularx}{\textwidth}{@{}p{0.16\textwidth}p{0.23\textwidth}p{0.23\textwidth}Y@{}}
\toprule
Family & Instruct member & Reasoning member & Routing / caveat \\
\midrule
DeepSeek V4-pro$^\dagger$
  & \begin{tabular}[t]{@{}l@{}}\texttt{deepseek-v4-pro}\\\texttt{deepseek/}\\\texttt{deepseek-v4-pro}\end{tabular}
  & \begin{tabular}[t]{@{}l@{}}\texttt{deepseek-v4-pro}\\\texttt{deepseek/}\\\texttt{deepseek-v4-pro}\end{tabular}
  & Same model; DS-direct recoveries use native \texttt{thinking} on/off, OR originals use legacy \texttt{reasoning}. \\
\midrule
DeepSeek V4-flash
  & \texttt{deepseek-chat}
  & \texttt{deepseek-reasoner}
  & DeepSeek direct aliases for instruct and reasoning paths. \\
\midrule
Qwen3-235B
  & \begin{tabular}[t]{@{}l@{}}\texttt{qwen/qwen3-}\\\texttt{235b-a22b-2507}\\(Novita)\end{tabular}
  & \begin{tabular}[t]{@{}l@{}}\texttt{qwen/qwen3-}\\\texttt{235b-a22b-thinking-}\\\texttt{2507} (Alibaba)\end{tabular}
  & Matched family and active-parameter count; upstream differs across siblings. \\
\midrule
Qwen3-30B
  & \begin{tabular}[t]{@{}l@{}}\texttt{qwen/qwen3-}\\\texttt{30b-a3b-instruct-}\\\texttt{2507} (Nebius)\end{tabular}
  & \begin{tabular}[t]{@{}l@{}}\texttt{qwen/qwen3-}\\\texttt{30b-a3b-thinking-}\\\texttt{2507} (AtlasCloud)\end{tabular}
  & Matched family and active-parameter count; upstream differs across siblings. \\
\midrule
anchor (no pair)
  & Claude Opus 4.7
  & --
  & Closed-frontier anchor provisioned but excluded from the paired tests. \\
\bottomrule
\end{tabularx}
\caption{Within-family pairs and upstream pinning. OpenRouter fallback
routing is disabled for all pinned requests. Within each Qwen pair the
default upstream host differs across siblings (a soft serving-stack
limitation; see \S\ref{sec:limitations}). $^\dagger$ The
\texttt{deepseek-v4-pro} pair is mixed after the 2026-05-15 amendment:
OR/Novita originals are kept where they succeeded, and failed cells are
recovered through DeepSeek-direct. See
\S\ref{sec:methods:v4pro-routing} for the per-axis breakdown.}
\label{tab:models}
\end{table}
...
The DeepSeek V4-pro pair was originally pinned to OpenRouter
...
DeepSeek-direct with the documented native thinking parameter and a
budget of $16{,}384$ tokens reproducibly returned a populated
\texttt{content} field on the same prompts. (ii)~\textbf{the
V4-family-native parameter is not honoured for the legacy reasoning
keyword.}\ DeepSeek documents V4 reasoning control through a native
thinking parameter; the older reasoning keyword is silently ignored.
Empirically, the older keyword continued to emit non-zero reasoning
tokens, whereas the native disabled setting produced reasoning tokens
of zero. (iii)~\textbf{methodological
...
\texttt{provider\_error} and \texttt{provider\_empty\_response}
originals via the semantic dedup at analysis time
(\S\ref{sec:methods:dedup}). Both attempts remain in the raw JSONL audit
trail. The analyzable set keeps OR/Novita originals where they
succeeded, plus DS-direct retries on cells where the OR-served original
failed.
...
\bottomrule
\end{tabular}
\caption{Post-dedup V4-pro routing mix. OR = original OpenRouter/Novita
row; DS = DeepSeek-direct recovery row.}
\label{tab:v4pro-routing-mix}
\end{table}

The mix is heaviest on the reasoning sycophancy cells precisely because
that was the cell type most affected by the \texttt{max\_tokens}{=}2048
bug.
...
in the analysis code and applied at analyzer read-time. Class~(i),
\texttt{provider\_error}, is
...
2026-05-14 amendment are retroactively reclassified from their token
...
sweep-cell, seed) cell. For the analyzable dataset we apply semantic
deduplication, which keys cells on
\emph{(model-family, role, task, sweep, seed)}---
