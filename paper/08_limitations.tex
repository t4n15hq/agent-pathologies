...
The original sweep produced a substantial empty-answer class
concentrated on DeepSeek reasoning members ($\approx\!668$ on
DeepSeek-reasoner and $\approx\!407$ on V4-pro reasoning during stage-1
calibration), at first treated as a model-side behavioural signal.
Diagnostic work on
...
32K for reasoning models, so the entire budget was consumed by the
reasoning trace, leaving zero tokens for the final visible answer. Per the same-day
...
A stage-2 Claude Opus 4.7 anchor is provisioned in the configuration
but is not part of the paired test
(\S\ref{sec:method:pairing}). The preregistration amendment dated
2026-05-13 records the staged rollout: stage 1 is the paired
...
Within-family pairs are restricted to open-weight families that ship
matched instruct and reasoning siblings — at time of writing, this is
the DeepSeek V4 family (within-MODEL reasoning toggle on shared
weights) and the Qwen3 family (cross-SKU at matched scale). We do not
claim the within-family pattern generalises to closed-frontier
reasoning models without further data.
