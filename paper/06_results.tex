...
\caption{Per-pair coverage. Triples are $N$ for (self-cons, sycophancy,
context-rot). Pre-rec.\ excl.\ are stage-1 empty-response counts on
DeepSeek reasoning before the 2026-05-15 \texttt{max\_tokens} amendment;
effective excl.\ counts cells where every attempt failed after dedup.
$^*$ within-Qwen-pair upstream mismatch (\S\ref{sec:setup});
$^\dagger$ V4-pro mixed OR/Novita + DS-direct recovery
(\S\ref{sec:methods:v4pro-routing}).}
\label{tab:pairs}
{\small\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lllccrr@{}}
\toprule
Family & Role & Upstream & $N_{\mathrm{cells}}$ & $N_{\mathrm{analyzable}}$ & pre-rec. excl. & effective excl. \\
\midrule
V4-flash  & instruct  & DeepSeek-direct  & $1000{+}600{+}1200$ & $1000{+}600{+}1200$ & $0$               & $0{+}0{+}0$ \\
V4-flash  & reasoning & DeepSeek-direct  & $1000{+}600{+}1200$ & $1000{+}600{+}1200$ & ${\approx}668$    & $0{+}0{+}0$ \\
V4-pro    & instruct  & mixed$^\dagger$  & $1000{+}600{+}1200$ & $1000{+}600{+}1200$ & $0$               & $0{+}0{+}0$ \\
V4-pro    & reasoning & mixed$^\dagger$  & $1000{+}600{+}1200$ & $1000{+}600{+}1200$ & ${\approx}407$    & $0{+}0{+}0$ \\
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.78\linewidth]{figures/fig1_headline_forest.pdf}
\caption{Headline paired effects across the three pathology axes;
positive$=$reasoning advantage. Self-consistency is the largest and
cleanest effect; sycophancy is heterogeneous; context-rot splits
between DeepSeek ceiling effects and Qwen capability-confounded gains.}
...
\emph{capability-recovery} comparisons in \S\ref{sec:discussion}: the
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig2_selfconsistency_paired.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig6_qwen_mode_collapse.pdf}
...
The reasoning advantage attenuates with post-pushback distance on the
cleanest pair: V4-flash's gain falls from $+0.240$ at $g=0$ to $+0.080$
at $g=10$, suggesting reasoning's sycophancy resistance is strongest
immediately after pushback and decays as filler accumulates.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.78\linewidth]{figures/fig3_sycophancy_did.pdf}
...
\vspace{0.6em}
\includegraphics[width=0.78\linewidth]{figures/fig4_sycophancy_conditions.pdf}
...
Context rot is split cleanly between two regimes. On the DeepSeek pairs
the instruct sibling already solves the $20$-update variable-tracking
task at $\sim 1.000$ accuracy across all filler depths, leaving no
within-pair headroom for reasoning to improve; we report this as a
ceiling-bound null. On the Qwen pairs the instruct sibling has room to
fail and does---Qwen3-30B instruct drops to $0.500$ accuracy at
$n_{\mathrm{filler}}=40$ under irrelevant filler, while the reasoning
sibling holds $0.960$ ($h=1.168$, BH-$q=1.9{\times}10^{-5}$). The Qwen
gain is large but partially entangled with the same hardness-$5$
capability gap documented in \S\ref{sec:results:selfconsistency}, so we
read the magnitude through the capability-confound lens
(\S\ref{sec:discussion:capability}) while treating the direction as
evidence of what context-rot recovery looks like when there is headroom.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig5_contextrot_curves.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig7_cross_axis_summary.pdf}
