...
% ---------- STRUCTURE NOTE ----------
% Six tables are pre-specified before final data lands; their layouts are
% sketched below as `\NUM` placeholders. Once `data/*_clean.jsonl` and
% `pair_analysis.csv` are produced (post-sweep), each `\NUM` is replaced
% with the corresponding cell value.
%
% Order of presentation is deliberately:
%   (T1) preliminaries (pairs + exclusions; necessary before any numbers
%        are interpretable);
%   (T2-T4) self-consistency and sycophancy, the two strong axes;
%   (T5) context rot, the mixed/weak axis, presented honestly;
%   (T6) cross-axis pass/fail summary.
...
Sycophancy is reported in two tables. Table~\ref{tab:syc:cell} gives the
raw per-cell paired accuracies, which support the cell-wise McNemar
diagnostic. Table~\ref{tab:syc:did} reports the headline statistic, the
between-model paired difference-in-differences (DiD)---$\big[\mathrm{reas}(\acc{c} -
\acc{w})\big] - \big[\mathrm{instr}(\acc{c} - \acc{w})\big]$---per family
and gap.
...
pushes against the correct answer—i.e., is \emph{more} sycophantic. So
\textbf{positive $\mathrm{gain}$ means reasoning has a smaller gap and is
\emph{less} sycophantic}. The CI is the 95\% bootstrap CI on
$\mathrm{gain}$ (equivalently, the negated-and-swapped CI on
$\ndid = \mathrm{reas\_gap} - \mathrm{instr\_gap}$). Preregistered effect
threshold: $|\mathrm{gain}| \geq 0.10$. BH-$q$ is adjusted across the four
...
overclaiming. Table~\ref{tab:ctx:cell} reports the raw per-cell paired
...
% TODO once data is in:
%   - replace every \NUM with the corresponding cell value from
%     pair_analysis.csv and the per-axis paired CSVs.
%   - drop in PNG figures from paper/figures/ (forest plot, divergence
%     histograms, decay curves, DiD by family).
%   - if any axis result contradicts the selective-intervention reading,
%     rewrite the closing paragraph of this section honestly rather than
%     reframing post-hoc.
